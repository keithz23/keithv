...
const links = [{ label: "About", id: "about" }, { label: "Work", id: "projects" }, { label: "Contact", id: "contact" }];
...
export default function Navbar() {
  ...
  useEffect(() => {
    const sections = ["home", ...links.map((link) => link.id)].map((id) => document.getElementById(id)).filter(Boolean) as HTMLElement[];
  ...
  return (
    <header className="h-20 border-b border-zinc-200 bg-white/80 dark:border-zinc-800 dark:bg-zinc-950/80">
      <nav className="fixed inset-x-0 top-0 z-40 border-b border-zinc-200/80 bg-white/75 backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,.65)] dark:border-zinc-800 dark:bg-zinc-950/75 dark:shadow-[inset_0_1px_0_rgba(255,255,255,.05)]" aria-label="Primary navigation">
        <div className="mx-auto grid h-20 max-w-7xl grid-cols-[1fr_auto] items-center px-4 sm:px-8 md:grid-cols-[1fr_auto_1fr] lg:px-10">
          <button type="button" onClick={() => go("home")} className="group flex w-fit items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
            <span className="grid h-9 w-9 place-items-center bg-zinc-900 font-mono text-xs font-semibold text-white transition-transform duration-300 group-hover:-rotate-3 group-active:scale-[.98] dark:bg-zinc-100 dark:text-zinc-950">KV</span>
            <span><span className="block text-sm font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">Keith Vuong</span><span className="block font-mono text-[9px] uppercase tracking-[.18em] text-zinc-400">Full-stack developer</span></span>
          </button>
